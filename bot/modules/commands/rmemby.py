...
async def rmemby_user(_, msg):
    ...
    if e is None:
        return await reply.edit(f"没有检索到 {b} 账户，请确认重试或手动检查。")
    ...
    if e.embyid is not None:
        ...
        if await emby_del_all(tg=e.tg, embyid=e.embyid):
            ...
            try:
                await reply.edit(
                    f'管理员 {sign_name} 已将 [{first.first_name}](tg://user?id={e.tg}) 账户 {e.name} 删除。')
                await bot.send_message(e.tg,
                                       f'管理员 {sign_name} 已将 您的账户 {e.name} 删除。')
            except:
                pass
            LOGGER.info(
                f"【admin】：管理员 {sign_name} 执行删除 {first.first_name}-{e.tg} 账户 {e.name}")
    else:
        await reply.edit(f"[ta](tg://user?id={b}) 还没有注册账户")
