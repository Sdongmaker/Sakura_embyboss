...
async def renew_all(_, msg):
    ...
    send = await bot.send_photo(msg.chat.id, photo=bot_photo, caption="【批量续期任务】\n  **正在执行...请稍后**")
    rst = get_all_emby(Emby.lv == 'b')
    if rst is None:
        LOGGER.info(
            f"【批量续期任务】 -{msg.from_user.first_name}({msg.from_user.id}) 没有检测到任何emby账户，结束")
        return await send.edit("【批量续期任务】\n\n结束，没有需要处理的账户")
    ...
    if sql_update_embys(some_list=ls, method='ex'):
        ...
        times = end - start
        await send.edit(
            f"【批量续期任务】\n  批量续期 {a} 天 * {b}，耗时：{times:.3f}s\n 时间已到账，正在向每个拥有emby的用户私发消息，短时间内请不要重复使用")
        LOGGER.info(
            f"【批量续期任务】 - {msg.from_user.first_name}({msg.from_user.id}) 批量续期 {a} 天 * {b} 更改用时{times:.3f} s")
        for l in ls:
            await bot.send_message(l[0], f"管理员 {msg.from_user.first_name} 调节了您的账户 到期时间：{a}天"
                                         f'\n实时到期：{l[1].strftime("%Y-%m-%d %H:%M:%S")}')
        LOGGER.info(
            f"【批量续期任务】 - {msg.from_user.first_name}({msg.from_user.id}) 批量续期 {a} 天 * {b}，消息私发完成")
    else:
